# Transforma o arquivo inserido em binário
def arquivo_para_binario(caminho_arquivo):
    with open(caminho_arquivo, 'rb') as arquivo:
        conteudo = arquivo.read()
        arquivo_bin = ''.join([bin(byte)[2:].rjust(8, '0') for byte in conteudo])
    return arquivo_bin

# Atualizada para aceitar direto o binário já montado
def tamanho_arquivo(arquivo_bin):
    return bin(len(arquivo_bin)).removeprefix('0b').rjust(64, '0')

# Não mudei o nome das variáveis do resto
...
def calcular_sha256(caminho_arquivo):
    # Realiza a leitura binária do arquivo e o cálculo do hash
    arquivo_binario = arquivo_para_binario(caminho_arquivo)
    tamanho_arquivo_bin = tamanho_arquivo(arquivo_binario)
    arquivo_padding = padding_juncao(arquivo_binario, tamanho_arquivo_bin)
    blocos = separa_bloco(arquivo_padding)
    ...
    sha = sha256(H, ultimas_variaveis)
    return sha